...
class MovieDataService(
	...
	fun save_MovieData_new(movieId : String): MovieData{
		...
		if (! movie_data_json.get("avgRating").equals(null)) {
			netizenAvgRate = movie_data_json.get("avgRating").toString().toDouble()
		} else {
			netizenAvgRate = 0.0
		}

		if (! movie_data_json.get("reservationInfo").equals(null)) {
			try {
				reservationRate =
					JSONObject(movie_data_json.get("reservationInfo").toString()).get("reservationRate").toString().toDouble()
			} catch (e: Exception) {
				reservationRate = 0.0
			}
		} else {
...
//		var admissionCode : String? = JSONObject(movie_data_json.get("countryMovieInformation").toString()).get("admissionCode").toString()
		...
		val member_MovieData = MovieData(
			movie_data_json.get("titleKorean").toString() + "_" + movie_releaseDate,
			netizenAvgRate,
			reservationRate,
			plot,
			NameKR,
